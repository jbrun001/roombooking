...
app.use((req, res, next) => {
  ...
  cSPolicy = cSPolicy + "script-src http://localhost:8000 'unsafe-inline' https://ajax.googleapis.com; "; // unsafe-inline allows for script in the pages
  cSPolicy = cSPolicy + "img-src 'self' https://cdn3.iconfinder.com https://i.imgur.com; ";
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With, If-Modified-Since");
...
function getBookings(pageName, userId, filters, listOrder) {
  return new Promise((resolve, reject) => {
    ...
    if (filters.date != "") sqlquery = sqlquery + ` 
    AND (
      b.booking_start >= DATE_ADD('` + filters.date + `', INTERVAL '00:01' HOUR_MINUTE) 
      AND 
      b.booking_end <= DATE_ADD('` + filters.date + `', INTERVAL '23:59' HOUR_MINUTE)
    ) 
    `
    ...
    if (filters.timeslot != "-NaN:NaN") {
      ...
      sqlquery = sqlquery + ` 
        AND (
          b.booking_start >= DATE_ADD('` + selectedDate + `', INTERVAL '` + startTime + `' HOUR_MINUTE) 
          AND 
          b.booking_end <= DATE_ADD('` + selectedDate + `', INTERVAL '` + endTime + `' HOUR_MINUTE)
        ) 
      `
    }
    // add any other filters to the end of the query
    if (filters.building != "") sqlquery = sqlquery + " AND r.building_name = '" + filters.building + "' ";
    if (filters.roomType != "") sqlquery = sqlquery + " AND r.room_type = '" + filters.roomType + "' ";
    if (filters.minSeats != "") sqlquery = sqlquery + " AND r.capacity >=  '" + filters.minSeats + "' ";
  ...
  return new Promise((resolve, reject) => {
    ...
    if (filters.date == "" || filters.timeslot == "-NaN:NaN") {
      ...
      AND r.id NOT IN (
        SELECT room_id
        FROM booking
        WHERE DATE(booking_start) = '` + selectedDate + `'
        AND (
              (
                booking_start < DATE_ADD('` + selectedDate + `', INTERVAL '` + startTime + `' HOUR_MINUTE) 
                AND 
...
app.get("/rooms-list", isLoggedIn, (req, res) => {
  ...
  Promise.all([
    ...
    .then(([roomTypes, buildingNames, rooms]) => {    // if you had more data just add the name of it here first variable is the result of promise.all[0] etc.
...
app.post("/rooms-list-filtered", isLoggedIn, function (req, res) {
  ...
  Promise.all([
    ...
    .then(([roomTypes, buildingNames, rooms]) => {           // if you had more data calls above you name it here, the first variable is the result of promise.all[0] etc.
      console.log(filters);
      res.render("rooms-list.ejs", {
        loggedInMessage,
        userrole,
...
app.post("/edit-room-success", isLoggedIn, (req, res) => {
  ...
  sqlquery = "UPDATE room SET room_number = ?, building_name = ?, room_type = ?, capacity = ?, picture_URL = ?, is_accepting_bookings = ? WHERE id = ?";
  db.query(sqlquery, (err, result) => {
    if (err) {
      console.error(err.message);
